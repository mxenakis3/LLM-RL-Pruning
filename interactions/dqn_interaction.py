...
from configs.dqn_LL_default import dqn_lunar_lander_default_configs_dict as dqn_ll_config
...
class DQNInteraction:
  ...
  def train(self):
    ...
    # INIT EPSILON
    epsilon = self.config.eps_start # Initial value for eps
    decay_type = self.config.decay_type

    # Figure out what the decay factor should be
    if decay_type == "Linear":

      # Find constant factor by which to decrease lambda
      eps_decrement = (self.config.eps_start - self.config.eps_end) / self.config.eps_decay
  ...
  def _get_environment(self, train, config):
