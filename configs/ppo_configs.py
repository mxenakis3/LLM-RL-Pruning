...
ppo_interaction_config_texas_ex = Config({
  # INTERACTION CONFIGURATIONS:
...
"kap_decay_episodes": 10, # For linear decay: The number of steps we are taking to decay kap. Each episode, kap decays by (kap_start - kap_finish)/ kap_decay_episodes.
...
ppo_interaction_config_texas_random = Config({
  # INTERACTION CONFIGURATIONS:
...
ppo_interaction_config_texas_heuristic = Config({
  # INTERACTION CONFIGURATIONS:
"training_episodes": 70000,
"testing_episodes": 3000,
"bot_type": "heuristic", # "heuristic" or "random"
"c": 0.3, # Loss function clipping coefficient
"batch_size":64
, # Size of batch for SGD in update method.
"capacity": 20000, # Size of experience tuple (s, a, r, s_, ...) memory
"num_epochs": 4, # number of batches in each iteration of learning
...
actor_configs_texas_heuristic = Config({
# AGENT CONFIGURATIONS
"learning_rate": 3e-5,
"loss_function": "mseloss",
"optimizer": "adam",
...
critic_configs_texas_heuristic = Config({
# INTERACTION CONFIGURATIONS:
